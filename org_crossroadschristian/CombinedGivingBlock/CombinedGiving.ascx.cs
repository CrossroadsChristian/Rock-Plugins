...
namespace RockWeb.Plugins.org_crossroadschristian.CombinedGivingBlock
{
    [DisplayName( "Combined Giving List" )]
    [Category( "Crossroads > Finance" )]
    [Description( "Builds a list of all financial transactions which can be filtered by date, account, transaction type, etc. This one is used to view Combined Family Giving." )]

    [ContextAware]
    [LinkedPage( "Detail Page" )]
    [TextField( "Title", "Title to display above the grid. Leave blank to hide.", false )]
    public partial class CombinedGiving : Rock.Web.UI.RockBlock, ISecondaryBlock, IPostBackEventHandler
    {
        #region Fields
...
", _ddlMove.ClientID, gTransactions.ClientID, Page.ClientScript.GetPostBackEventReference( this, "MoveTransactions" ) );
        ...
        private void BindGrid()
        {
            ...
            var qryList = new List<FinancialTransaction>();
            string familyGroupFullNames = String.Empty;
            ...
            else    // Person
            {
                ...
                if ( _person != null )
                {
                    List<GroupMember> familyGroup = _person.GetFamilyMembers(true).ToList(); //Gets all the family members
                    var givingGroup = new List<Person>();
                    var givingSeparately = new List<Person>();

                    if ( familyGroup.Any() )
                    {
                        if ( _person.GivingGroup != null )
                        {
                            if ( _person.GivingGroupId != null )
                            {
                                foreach ( var member in familyGroup )
                                {
                                    //qry = qry.Where( t => t.AuthorizedPersonAlias.PersonId == person.PersonId );
                                    int personId = member.Id;

                                    Person person = member.Person;
                                        //new PersonService( new RockContext() ).Queryable().Where(p => p.Id == personId).FirstOrDefault();
                                    if ( person.GivingGroup != null && person.GivingGroupId == _person.GivingGroupId )
                                    {
                                        givingGroup.Add( person );
                                    }
                                    else
                                    {
                                        givingSeparately.Add( person );
                                    }
                                }
                                foreach ( var person in givingGroup )
                                {

                                    qry = qry.Where( t => t.AuthorizedPersonAlias.PersonId == person.Id );
                                    if ( qry != null )
                                    {
                                        qryList.AddRange( qry.AsNoTracking().ToList() );
                                    }

                                    familyGroupFullNames += person.FullName + ", ";
                                }
                            }
                            else
                            {
            ...
            if ( qryList.Any() )
            {
                gTransactions.DataSource = qryList.OrderByDescending(t => t.TransactionDateTime).ToList();
                
            }
            familyGroupNames.Text = familyGroupFullNames;
